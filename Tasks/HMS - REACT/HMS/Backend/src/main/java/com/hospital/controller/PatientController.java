...
public class PatientController {
	...
	@GetMapping("/gender")
	public ResponseEntity<Map<String, List<String>>> getGenderOptions() {
		try {
			List<String> genders = List.of("Male", "Female", "Other");
			Map<String, List<String>> response = new HashMap<>();
			response.put("gender", genders);  // Changed from "genders" to "gender"
	...
	public ResponseEntity<Map<String, List<String>>> getBloodGroupOptions() {
		try {
			List<String> bloodGroups = List.of("A+", "A-", "B+", "B-", "AB+", "AB-", "O+", "O-");
			Map<String, List<String>> response = new HashMap<>();
			response.put("bloodGroup", bloodGroups);  // Changed from "bloodGroups" to "bloodGroup"
			return ResponseEntity.ok(response);
		} catch (Exception e) {
			return ResponseEntity.status(500).body(null);
		}
	...
	public ResponseEntity<Map<String, List<String>>> getConditionOptions() {
		try {
			List<String> conditions = List.of("Severe", "Normal", "Critical", "Stable");
			Map<String, List<String>> response = new HashMap<>();
			response.put("condition", conditions);  // Changed from "conditions" to "condition"
			return ResponseEntity.ok(response);
		} catch (Exception e) {
			return ResponseEntity.status(500).body(null);
		}
