...
import { TimeSlotAction } from '../../store/timeSlots/timeSlots.actions';
import { selectAvailableTimeSlot, selectBookedTimeSlot, selectTimeSlotLoading } from '../../store/timeSlots/timeSlots.selectors';
import { LoadingBarComponent } from '../loading-bar/loadingBar.component';
...
export class AppointmentsComponent implements OnInit {
  ...
  availableTimeSlots$ = this.store.select(selectAvailableTimeSlot);
  bookedTimeSlots$ = this.store.select(selectBookedTimeSlot);
  timeSltLoading$ = this.store.select(selectTimeSlotLoading);
  ...
  private updateTimeSlots() {
    ...
    if (selectedDoctor?.id) {
      ...
      const base = `doctors/${selectedDoctor.id}`;
      const avlbSlotUrl = `${base}/available-slots?date=${dateStr}`;
      const bookedSlotUrl = `${base}/booked-slots?date=${dateStr}`;
      this.store.dispatch(TimeSlotAction.getAvailableTimeSlots({ avlbSlotUrl }));
      this.store.dispatch(TimeSlotAction.getBookedTimeSlots({ bookedSlotUrl }));

      this.availableTimeSlots$.subscribe({
        next: (availableSlots) => {
          ...
          this.bookedTimeSlots$.subscribe({
            next: (bookedSlots) => {
  ...
  private buildTimeSlotsFromAppointments() {
    ...
    const endHour = this.meta?.timeSlotConfig?.endHour || 11;
