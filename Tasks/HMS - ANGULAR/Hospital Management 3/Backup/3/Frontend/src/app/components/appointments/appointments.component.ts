...
export class AppointmentsComponent implements OnInit {
  ...
  private initializeForm() {
    ...
    this.bookingForm = this.fb.group({
      patient: [this.isPatientRole ? currentUser!.id : '', Validators.required],
  ...
  private updateTimeSlots() {
    ...
    if (selectedDoctor?.id) {
      ...
      available$.subscribe({
        next: (availableSlots) => {
          const available = (availableSlots || []).map(s => ({ time: s.timeSlot, isBooked: false, isSelected: false }));
          booked$.subscribe({
            next: (bookedSlots) => {
              const booked = (bookedSlots || []).map(s => ({ time: s.timeSlot, isBooked: true, isSelected: false }));
              this.timeSlots = [...available, ...booked].sort((a, b) => a.time.localeCompare(b.time));
              if (this.selectedSlot?.isBooked) this.selectedSlot = null;
            },
            error: () => {
